...
from fuzzy_evaluate import FuzzyEvaluate
...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.flow_stats = {}
        self.flow_speed = {}
        self.flow_size = {}  # {(ip_src,ip_dst):[speed,],}
        self.stats = {}
        self.port_features = {}
        ...
        self.best_paths = {}  # {(ip_src,ip_dst):path,}
        ...
        self.fuzzy_excutor = FuzzyEvaluate()
    ...
    def _monitor(self):
		...
		"""
        while CONF.weight == 'fuzzy' or CONF.weight == 'hop':
            # Refresh data.
            self.stats['flow'] = {}
            self.stats['port'] = {}
            self.capabilities = None
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if (
            ...
            self.save_flow_size(key[1], key[2], speed)
            ...
            self.flows.append({'src': src, 'dst': dst, 'demand': 0,
        ...
        if len(self.statRecord) == 1.25 * (CONF.fanout ** 2) and self.flows:
            ...
            link_src, link_dst = self.get_busy_link()
            sw_src, sw_dst = self.get_rescheduled_flow(link_src, link_dst)
            if sw_src == 0 or sw_dst == 0:
                return

            ip_src, ip_dst = self.host_peer[(sw_src,sw_dst)]

            if ip_src == 0 or ip_dst == 0:
                return

            flow = {}

            for f in self.flows:
                if f['src'] == ip_src and f['dst'] == ip_dst:
                    flow = f
                    break

            shortest_paths = self.awareness.shortest_paths
            graph = self.awareness.graph
            self.reroute(graph, shortest_paths, sw_src, sw_dst, flow)
        else:
            pass

    def get_busy_link(self):
        link = {}
        link['src'] = 0
        link['dst'] = 0
        ...
        for dpid in self.free_bandwidth:  # {dpid:{port_no:free_bw,},} Unit:Kbit/s
            for port in self.free_bandwidth[dpid]:
                bw_free = self.free_bandwidth[dpid][port] / 10000
                if not self.awareness.link_src_dst.has_key((dpid, port)):
                    continue
                if bw_free < 0.2 and bw_free < min_bw_free:
                    min_bw_free = bw_free
                    link['src'] = dpid
                    link['dst'] = self.awareness.link_src_dst[(dpid, port)][
                        0]  # {(src_dpid,src_port):(dst_dpid,dst_port),}

        return link['src'], link['dst']

    def get_rescheduled_flow(self, link_src, link_dst):

        max_speed = 0
        res = []

        for sw_src in self.best_paths.keys():
            for sw_dst in self.best_paths[sw_src]:
                path = self.best_paths[sw_src][sw_dst]
                # print path
                for i in range(1, len(path)):
                    if path[i - 1] == link_src and path[i] == link_dst and self.flow_size[(sw_src, sw_dst)] > max_speed:
                        res = (sw_src, sw_dst)
                        max_speed = self.flow_size[(sw_src, sw_dst)]
        if len(res) == 0:
            return 0, 0
        return res[0], res[1]

    def reroute(self, graph, shortest_paths, src, dst, flow):
        path = self.get_best_path_by_fuzzy(graph, shortest_paths, src, dst)
        print '*******************reroute**********************'
        print path
        match = flow['match']
        flow_info = (match['eth_type'], match['ipv4_src'], match['ipv4_dst'], flow['priority'])
    ...
    def save_flow_size(self, ip_src, ip_dst, speed):
        _dict = self.flow_size
        key = self.sw_peer[(ip_src,ip_dst)] # sw_peer{(ip_src,ip_dst):(sw_src,sw_dst)}
        # sw_peer = self.get_sw()
        if key not in _dict:
            _dict[key] = []
        _dict[key].append(speed)
        if len(_dict[key]) > 5:
            _dict[key].pop(0)

    def save_sw_speed(self):
        sw_speed = {}
        for dpid in self.flow_speed.keys():
            for key in self.flow_speed[dpid].keys():
                if sw_speed.has_key(dpid):
                    sw_speed[dpid] += self.flow_speed[dpid][key][0]
                else:
                    sw_speed[dpid] = self.flow_speed[dpid][key][0]
        self.sw_speed = sw_speed
    ...
    def get_best_path_by_bw(self, graph, paths):
        ...
        return best_paths  # capabilities,
    ...
    def get_best_path_by_fuzzy(self, graph, shortest_paths, src, dst):
        """
            Get best path by comparing paths.
            Note: This function is called in FuzzyLogic module.
        """
        best_paths = copy.deepcopy(shortest_paths)
        paths = shortest_paths[src][dst]

        if not self.best_paths.has_key(src):
            self.best_paths[src] = {}

        if not self.best_paths[src].has_key(dst):
            self.best_paths[src][dst] = []
        ...
        for path in paths:
            min_bw = setting.MAX_CAPACITY
            min_bw = self.get_min_bw_of_links(graph, path, min_bw)
            bw_used = 1 - (min_bw * 1.0 / setting.MAX_CAPACITY)
            hops = len(path)
            val = self.fuzzy_excutor.fuzzy_evaluate(bw_used, hops)
            if val < result:
                best_path = path
                result = val
        self.best_paths[src][dst] = best_path
    ...
    def get_hop_weight(self, hops):
        hop_weight = []
        hop_weight.append(self.hop_short(hops))
        hop_weight.append(self.hop_long(hops))
        return hop_weight
    ...
    def L(self, x):
        ...
        if x<=0.2:
            ...
            return 0
        '''''
        return self.gauss_fun(0.35, 0.1, x)

    def M(self, x):
        # return L(x-0.2)
        return self.gauss_fun(0.55, 0.1, x)

    def H(self, x):
        # return L(x-0.4)
        return self.gauss_fun(0.75, 0.1, x)
    ...
    def get_bw_weight(self, bw_used):
        bw_weight = []
        bw_weight.append(self.LL(bw_used))
        bw_weight.append(self.L(bw_used))
        bw_weight.append(self.M(bw_used))
        bw_weight.append(self.H(bw_used))
        bw_weight.append(self.HH(bw_used))
        return bw_weight
    ...
    def res_M(self, x):
        return self.res_L(x - 0.2)

    def res_H(self, x):
        return self.res_L(x - 0.4)

    def gauss_fun(self, u, sig, x):
        # u = 0  # 均值μ
        # u01 = -2
    ...
    def fuzzy_evaluate(self, bw_used, hops):
        hop_weight = self.get_hop_weight(hops)
        bw_weight = self.get_bw_weight(bw_used)
    ...
    def update_fun(self, x, res):
        return max(min(self.res_L(x), res[0]), min(self.res_M(x), res[1]), min(self.res_H(x), res[2]))
