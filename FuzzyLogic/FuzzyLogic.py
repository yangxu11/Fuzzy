...
class ShortestForwarding(app_manager.RyuApp):
	...
	WEIGHT_MODEL = {'hop': 'weight', 'fuzzy': 'fuzzy'}
	...
	def get_path(self, src, dst, weight):
		...
		if weight == self.WEIGHT_MODEL['hop']:
		...
		elif weight == self.WEIGHT_MODEL['fuzzy']:
			...
			#result = self.monitor.get_best_path_by_fuzzy(graph, shortest_paths)
			result = self.monitor.get_best_path_by_fuzzy(graph, shortest_paths,src,dst)
	...
	def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst):
		...
		self.monitor.sw_peer[(ip_src,ip_dst)] = (result[0],result[1])
		self.monitor.host_peer[(result[0],result[1])] = (ip_src,ip_dst)
		if result:
