...
def traffic_generation(net, topo, flows_peers):
	...
	for src, dest in flows_peers:
		...
		# client.cmd("iperf -c %s -t %d -b %fM > /dev/null &" % (server.IP(), 200,random.randint(40,50)*1.0/10))   # Its statistics is useless, just throw away. 1990 just means a great number.
		#client.cmd("iperf -c %s -t %d -b 1M > /dev/null &" % (server.IP(), 200))   # Its statistics is useless, just throw away. 1990 just means a great number.
		#client.cmd("iperf -c %s -t %d -w %fM > /dev/null &" % (server.IP(), 200,random.randint(20,50)*1.0/10))   # Its statistics is useless, just throw away. 1990 just means a great number.
		client.cmd("iperf -c %s -t %d -w 3M> /dev/null &" % (server.IP(), 1900))   # Its statistics is useless, just throw away. 1990 just means a great number.
...
def run_experiment(pod, density, ip=ip_setting.ip, port=6653, bw_c2a=10, bw_a2e=10, bw_e2h=10):
	"""
	...
	fanout = args.k
	Controller_Ryu = Popen("ryu-manager --observe-links ./FuzzyLogic/FuzzyLogic.py --k_paths=%d --weight=fuzzy --fanout=%d" % (k_paths, fanout), shell=True, preexec_fn=os.setsid)
