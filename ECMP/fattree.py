...
#在启动程序demo.py时，在终端中输入 ./demo.py --k 8  就会将k这个选项的参数设置为8，不给参数时使用default参数4。
...
class Fattree(Topo):
	...
	"""
	CoreSwitchList = [] #核心交换机
	AggSwitchList = [] #汇聚层交换机
	EdgeSwitchList = [] # 边缘交换机
	HostList = []

	def __init__(self, k, density):
		self.pod = k
		self.density = density
		self.iCoreLayerSwitch = (k/2)**2 #乘方
...
def traffic_generation(net, topo, flows_peers):
	...
	for src, dest in flows_peers:
		...
		# client.cmd("iperf -c %s -t %d -b %fM > /dev/null &" % (server.IP(), 1990, random.randint(40, 50) * 1.0 / 10))
