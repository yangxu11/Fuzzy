...
def traffic_generation(net, topo, flows_peers):
	...
	for src, dest in flows_peers:
		...
		# client.cmd("iperf -c %s -t %d -b %fM > /dev/null &" % (server.IP(), 1990, random.randint(10, 30) * 1.0 / 10))
		# client.cmd("iperf -c %s -t %d -b 1M > /dev/null &" % (server.IP(), 200))
		client.cmd("iperf -c %s -t %d -w 1M > /dev/null &" % (server.IP(), 150))
		time.sleep(2)
...
def run_experiment(pod, density, ip=ip_setting.ip, port=6653, bw_c2a=10, bw_a2e=10, bw_e2h=10):
	...
	fanout = args.k
	Controller_Ryu = Popen("ryu-manager --observe-links ./Hedera/Hedera.py --k_paths=%d --weight=bw --fanout=%d" % (k_paths, fanout), shell=True, preexec_fn=os.setsid)
